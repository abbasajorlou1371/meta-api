...
class VideoSubCategory extends Model implements Sitemapable
{
    ...
    {
        return [
            Url::create('https://rgb.irpsc.com/fa/education/category/' . $this->category->slug . '/' . $this->slug)
                ->setLastModificationDate(Carbon::create($this->updated_at))
                ->setChangeFrequency(Url::CHANGE_FREQUENCY_MONTHLY)
                ->setPriority(0.8)->addImage($this->image_url),
            Url::create('https://rgb.irpsc.com/en/education/category/' . $this->category->slug . '/' . $this->slug)
                ->setLastModificationDate(Carbon::create($this->updated_at))
                ->setChangeFrequency(Url::CHANGE_FREQUENCY_MONTHLY)
                ->setPriority(0.8)->addImage($this->image_url)
        ];
